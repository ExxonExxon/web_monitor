...
| 🔒 Locked (Phase D) | SQLite via `aiosqlite` with **raw SQL in a repository layer**, no ORM (domain models are Pydantic — avoid dual mapping). Schema is idempotent `CREATE TABLE IF NOT EXISTS` applied on connect; `PRAGMA journal_mode=WAL`, `foreign_keys=ON`, `busy_timeout=5000`, `user_version` for schema version | §5.1, D6 |
| 🔒 Locked (Phase D) | Dispatcher owns fetch/retry/backoff/status transitions; transport is injected as `DeliveryHandler = (DomainEvent, ChannelConfig) -> Awaitable[None]`. Phase E supplies the Envelope + provider handler. Retry defaults: `max_attempts=5`, exponential backoff base 30s capped at 3600s, per-delivery (per-channel) independence | §6.2–§6.4 |
| 🔒 Locked (Phase D) | Timestamps persisted as ISO-8601 UTC text; JSON columns via Pydantic `model_dump_json`/`model_validate_json`; all bookkeeping timestamps use the injected clock. `Settings` gains `db_path=data/web_monitor.db` and `retention_days=30` | §5.1, §8-8 |
| 🔒 Locked (Phase D) | Runtime assembly (app lifespan opening the DB + starting scheduler/dispatcher) is deferred to Phase E; Phase D ships tested components (`Store`, `rehydrate`, `prune_checks`, `Dispatcher`) with no `app/main.py` change | §12.4, §15 |
